  ...
  const handleSubmit = async (formData: FormData) => {
    startTransition(async () => {
      ...
      if (result.success) {
        ...
        setError(result.error || "Помилка при збереженні рецепта");
      }
    });
  };
  ...
  return (
    <Form className="w-[450px]" action={handleSubmit}>
      ...
      <Input
        ...
        validate={(value) => (!value ? "Назва обов'язкова" : null)}
      />
            ...
            <Input
              ...
              validate={(value) =>
                !value || parseFloat(value) <= 0
                  ? "Кількість повинна бути більше за 0"
                  : null
              }
            />
      ...
      <div className="flex w-full items-center justify-end mt-4">
        <Button color="primary" type="submit" isLoading={isPending}>
          {initialRecipe ? "Зберігти зміни" : "Додати рецепт"}
        </Button>
      </div>
    </Form>
  );
