...
export const useRecipeStore = create<IRecipeState>((set) => ({
	...
	loadRecipes: async () => {
		...
		try {
			...
			set({ error: 'Помилка при завантаженні рецептів', isLoading: false })
		}
	},
	addRecipe: async (formData: FormData) => {
		...
		try {
			...
			console.error('error', error)
			set({ error: 'Помилка при додаванні рецепта', isLoading: false })
			return { success: false, error: 'Помилка при додаванні рецепта' }
		}
	},
	updateRecipe: async (id: string, formData: FormData) => {
		...
		try {
			...
			console.error('error', error)
			set({ error: 'Помикла при обновленні рецепта', isLoading: false })
			return { success: false, error: 'Помикла при обновленні рецепта' }
		}
	},
	removeRecipe: async (id: string) => {
		...
		try {
			...
			console.error('error', error)
			set({ error: 'Помилка при видаленні рецепта', isLoading: false })
		}
	}
}))
